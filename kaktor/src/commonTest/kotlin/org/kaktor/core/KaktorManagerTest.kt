...
class KaktorManagerTest {
    ...
    @Test
    fun `when an actor is created, it's saved on the manager map`() =
        runTest {
            val actorReference =
                kaktorManager.createActor(
    ...
    @Test
    fun `when an actor is attempted to be created again, it returns a new reference`() =
        runTest {
            val actorReference1 =
                kaktorManager.createActor(
                    ActorRegisterInformation(actorClass = MockKaktor::class) {
    ...
    @Test
    fun `when a message is sent that that actor doesn't recognize, it should be sent to error channel`() =
        runTest {
            val actorReference =
                kaktorManager.createActor(
                    ActorRegisterInformation(actorClass = MockKaktor::class) {
    ...
    @Test
    fun `when a message is sent to an unregistered actor, should throw ActorNotRegisteredException`() =
        runTest {
    ...
    @Test
    fun `when ask method is called, response should be received from the actor`() =
        runTest {
            val actorReference =
                kaktorManager.createActor(
                    ActorRegisterInformation(actorClass = MockKaktor::class) {
    ...
    @Test
    fun `when asked twice but first times out, first answer should be null, second should succeed`() =
        runTest {
            val actorReference =
                kaktorManager.createActor(
                    ActorRegisterInformation(actorClass = MockKaktor::class) {
    ...
    fun `when ask method is called for the property value, property should match the registration parameter`() =
        runTest {
            ...
            assertTrue { response2 == MOCK_ACTOR_DEFAULT_PARAMETER }
        }
}
