...
internal const val MOCK_ACTOR_DEFAULT_PARAMETER = 1

class MockKaktor(
    private val property1: Int = MOCK_ACTOR_DEFAULT_PARAMETER,
) : Kaktor<Command>() {
...
data class RemoveBalance(
    override val accountId: String,
    val value: Long,
) : AccountCommand

data class RetrieveBalance(
    override val accountId: String,
) : AccountCommand
...
class AccountActor : Kaktor<AccountCommand>() {
    ...
    companion object {
        fun shardByAccountId(message: AccountCommand): String = message.accountId
    }
}
